...
int64 suffix_dfs(Trie* u, int depth) {
    ...
    for(int i = 0; i < 26; i++) {
        ...
        ans += suffix_dfs(u->edges[i], depth + 1);
    }
    return ans;
}

int64 preffix_dfs(Trie* u, int first_level = false) {
    int64 ans = 0;
    
    for(int i = 0; i < 26; i++) {
        if(u->edges[i] == NULL) {
            ans += first_level ? 0 : starts_with[i];
            continue;
        }

        ans += preffix_dfs(u->edges[i]);
    }
...
int main() {
    ...
    while(cin >> P >> S) {
        ...
        int64 ones = suffix_dfs(&suffix, 0);
        int64 ans = preffix_dfs(&preffix, true) + preffix_count * ones;
