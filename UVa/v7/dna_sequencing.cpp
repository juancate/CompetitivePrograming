...
int main() {
    ...
    while(gets(line)) {
        string str(line);
        len[0] = (int)strlen(line);

        gets(line);
        len[1] = (int)strlen(line);
        ...
        for(int i = 2; i < sa.n; i++) {
            if(parent[i] != parent[i-1]) {
                if(sa.LCP[i] > max_lcp && sa.LCP[i] > 0) {
                    max_lcp = sa.LCP[i];

                    ans.clear();
                    string s;
                    for(int k = 0; k < max_lcp; k++) {
                        s.push_back(sa.T[sa.SA[i] + k]);
                    }
                    ans.insert(s);
                }
                else if(sa.LCP[i] == max_lcp) {
                    string s;
                    for(int k = 0; k < max_lcp; k++) {
                        s.push_back(sa.T[sa.SA[i] + k]);
                    }
