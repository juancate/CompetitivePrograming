...
bool check_best(const State& next) {
    if(next.time < minDist[next.u][next.turbo_distance][next.turbos]) {
        minDist[next.u][next.turbo_distance][next.turbos] = next.time;
        return true;
    }
    return false;
}
        ...
        for(int i = 0; i < (int)G[current.u].size(); i++) {
            ...
            if(is_castle(v.first)) {
                ...
                if(v.second <= L && current.turbos > 0) {
                    State next = State(v.first, current.time, current.turbos-1, 0);
                ...
                // no use turbo
                State next = State(v.first, current.time + v.second, current.turbos, 0);
                //check best
                if(check_best(next))
                    Q.push(next);
            }
            else { // ville
                if(current.turbo_distance >= v.second) {
