...
class PlatypusPaternity {
public:
  bool compatible(string p, string s) {
    int n = p.size();
    REP(i, n)
      if(s[i] == 'Y' and p[i] == 'N') return false;
    return true;
  }
  ...
  int maxFamily(vector <string> female, vector <string> male, vector <string> sibling) {
    ...
    REP(i,F) {
      REP(j, S) if(compatible(female[i], sibling[j])) {
        REP(k, M) if(compatible(male[k], sibling[j])) {
          int sum = count(ALL(sibling[j]), 'Y') + 2;
