...
public:
  ...
  long long rec(int xMe, int yMe, int xHome, int yHome, vector <string> teleports) {
    long long d = dist(xMe, yMe, xHome, yHome);
    int n = teleports.size();
    REP(i, n) {
      int xs, ys, xe, ye;
      istringstream(teleports[i]) >> xs >> ys >> xe >> ye;
      vector<string> t;
      REP(j,n) if(i != j) t.push_back(teleports[j]);

      d = min(d, dist(xMe, yMe, xs, ys) + 10 + rec(xe, ye, xHome, yHome, t));
      d = min(d, dist(xMe, yMe, xe, ye) + 10 + rec(xs, ys, xHome, yHome, t));
    }
    return d;
  }

  int shortestDistance(int xMe, int yMe, int xHome, int yHome, vector <string> teleports) {
    return (int)rec(xMe, yMe, xHome, yHome, teleports);
  }
