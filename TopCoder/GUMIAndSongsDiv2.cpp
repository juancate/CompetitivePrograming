...
long long solve(int last, int mask) {
  ...
  memo[last][mask] = INFLL;
  for (int i = 0; i < n; i++) {
    if (mask & (1<<i)) {
      long long spent = d[i];
      if (mask != ((1<<n)-1)) spent += abs(t[i]-t[last]);
      memo[last][mask] = min(memo[last][mask], spent+solve(i, mask & (~(1<<i))));
    }
  }
  return memo[last][mask];
}
...
int GUMIAndSongsDiv2::maxSongs(vector <int> duration, vector <int> tone, int T) {
  ...
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < (1<<n); j++) {
      if (memo[i][j] != -1 && memo[i][j] <= T) {
        int curr = __builtin_popcount(j);
        best = max(best, curr);
      }
    }
  }
